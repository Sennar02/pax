...
static const u64 ACTORS_COUNT = 20u;
...
/*
void
move(f32 step, Vec2f orient, u64 actor, Array<Position>& position, Array<Movement>& movement, Array<Tile>& grid)
{
    Vec2f dist = {};
    Vec2f incr = {};
    Vec2u next = {};
...
*/
    ...
    for ( u64 i = 0; i < WORLD_SIZE[1]; i += 1u ) {
        for ( u64 j = 0; j < WORLD_SIZE[0]; j += 1u ) {
            index = i * WORLD_SIZE[0] + j;
            index = actor_layer.grid[index];
